...
def evaluate_board(grid, couleur: int):
    ...
    if roi:
        ...
        king_safety -= 20 * len([elem for elem in chess_utils.liste_pieces_dans_rayon(grid, roi.x, roi.y, 3) if
...
def iterative_deepening_negamax(board, couleur, final_depth, time_limite=120):
    ...
    best_score, best_move = alpha_beta_pruning(board, final_depth, color=couleur, alpha=-float("inf"), beta=float("inf"))
